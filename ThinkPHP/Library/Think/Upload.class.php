...
class Upload {
    ...
    public function save_thumb($img,$max){
        ...
        $url = '/Public/uploads/thumb/'.time() . rand(100, 999) . '.png';
        if($max){
            if ($array[0] > $max) {
                $imgs = new \Think\Image();
                $image = $imgs->open('.'.$img);
                // 按照原图的比例生成一个最大为500*500的缩略图并保存为thumb.png
                $image->thumb($max, $max)->save('.' . $url);
                return $url;
            }
        }
        return $img;
     ...
     * 多文件上传和单文件上传
     */
    public function uploads($dirname=''){
        $images = array();
        $config = [
            'maxSize'   => 500*3145728,
        ...
        if ($info){
            foreach($info as $file){
                array_push($images,C('IMG_PREFIX').'/Public/admin/Uploads/touxiang/'.$file["savename"]);
            }
        }else {
