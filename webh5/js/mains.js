...
app.controller('mainCtrl', ['$scope', '$rootScope', '$location', '$timeout', '$http', '$cookies', '$cookieStore', '$sce', function($scope, $rootScope, $location, $timeout, $http, $cookies, $cookieStore, $sce) {
    console.log('主控制器');
    angular.element("#loading").fadeIn(300);
    /*获取用户信息*/
    ...
    $http.post(
        ...
        if(data["status"] == 'ok'){
            ...
            $scope.goods_url_desc = $sce.trustAsHtml(unescapeHTML(data["data"].goods_detail));
            $scope.getmerth();
