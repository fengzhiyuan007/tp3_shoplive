  	...
  	link: function($scope,$element, $attributes){
  		$element.click(function(){
  			...
  			if($scope.orderType==1){ // 取消订单
  				$scope.api = 'app/Order/cancelOrder';
  			}else if($scope.orderType==2){ // 确认收货
  				$scope.api = 'app/Order/receiveOrder';
  			}else if($scope.orderType==3){ // 删除订单
  				$scope.api = 'app/Order/delOrder';
  			}else if($scope.orderType==4){ // 退款
          $scope.api = 'app/Order/return_order';
        }else if($scope.orderType==5){ // 催单
          $scope.api = 'app/Order/hurry_order';
        }
  			$http.post(
		      url + $scope.api,$.param({
  ...
  return {
    ...
    link: function($scope,$element, $attributes){
      $element.click(function(){
        ...
        $http.post(
          url + 'app/Mall/goods_collect',$.param($scope.paramObj),
          {headers: {'Content-Type' : 'application/x-www-form-urlencoded; charset=UTF-8'}}
        ).success(function(data){
          console.log(data);
          if(data["status"] == 'ok'){
