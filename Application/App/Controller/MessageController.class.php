...
class MessageController extends CommonController {
    ...
    public function message_list(){
        ...
        switch ($type){
            case 1:
                $list = M('Message')->where(['type'=>$type,'user_id2'=>$user['user_id']])->order('intime desc')->page($page,$pageSize)->field('message_id,type,user_id,user_id2,content,state,intime,date,title')->select();
                if ($list){
                    foreach ($list as $k=>$v){
                        if ($v['state']==1){
