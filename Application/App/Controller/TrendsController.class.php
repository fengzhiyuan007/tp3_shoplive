...
class TrendsController extends CommonController {
 ...
 //    }
    ...
    public function trends_list(){
        ...
        //     if ($log && $lag){
        //         foreach ($li as $k=>$v){
        //             $li[$k]['distance'] = (string)round(getDistance($lag,$log,$v['lag'],$v['log'])/1000,2);   //算出距离（公里），保留两位小数点(四舍五入)
        //         }
        //         foreach ($li as $k=>$v){
        //             if ($li[$k]['distance']>200){
        //                 unset($li[$k]);
        //             }
        //         }
        //     }
        ...
        //     $li = $this->two_sort($li,'distance','SORT_ASC');
        //     if(empty($li)){
        //         $li = [];
        //     }
            ...
            default:
                ...
                if ($log && $lag){
                    ...
                    foreach ($list as $k=>$v){
                        ...
                        $list[$k]['img_url'] ? $list[$k]['img_url']=domain($list[$k]['img_url']):"";
                        $list[$k]['header_img'] ? $list[$k]['header_img']=domain($list[$k]['header_img']):"";
                ...
                // $list = array_slice($list,$pageSize*($page-1),$pageSize);
    ...
    public function trends(){
        ...
        $trends['img_url'] ? $trends['img_url']=C('IMG_PREFIX').$trends['img_url']:"";
        $trends['header_img'] ? $trends['header_img']=C('IMG_PREFIX').$trends['header_img']:"";
        //赞的人数，赞的状态，评论人数
        $trends['zan'] = M('trends_zan')->where(array('trends_id'=>$trends['id'],'status'=>'1'))->count();
        $trends['zan_status'] = M('trends_zan')->where(array('trends_id'=>$trends['id']))->getField('status');
    ...
    public function get_qiniu_token(){
        ...
        // echo json_encode(['uptoken'=>$upToken,'url'=>'http://play.100ytv.com/']);
        success(['uptoken'=>$upToken,'url'=>'http://dspxplay.tstmobile.com/']);
    }

    public function tag_list(){
        $list = M('tag')->field('id,name,(count+counts) as count')->order('count desc')->select();
        success($list);

    }

    public function tag_new(){
        $list = M('tag')->where(array('type'=>1))->field('id,name,(count+counts) as count')->order('count desc')->select();
        success($list);
    }
}