            ...
            if ($user){
              ...
              //注册新用户
                ...
                if ($id=M('User')->add($date)){
                   ...
                   foreach ($no as $k=>$v){
                       M('Message')->add(['type'=>1,'user_id2'=>$id,'title'=>$v['title'],'content'=>$v['content'],'intime'=>time(),'date'=>date('Y-m-d',time())]);
                   }
                ...
                switch ($state) {
                    case '1':
                        if(empty($user['openid']) || empty($user['unionid'])){
                            M('user')->where(['user_id'=>$user['user_id']])->setField(['openid'=>$openid,'unionid'=>$unionid]);
                            $u = M('user')->where(['user_id'=>$user['user_id']])->find();
                            $u['img'] = domain($u['img']);
                    ...
                    case '2':
                        if(empty($user['qq_openid'])){
                            M('user')->where(['user_id'=>$user['user_id']])->setField('qq_openid',$openid);
                            $u = M('user')->where(['user_id'=>$user['user_id']])->find();
                            $u['img'] = domain($u['img']);
                    ...
                    case '3':
                        if(empty($user['weibo'])){
                            M('user')->where(['user_id'=>$user['user_id']])->setField('weibo',$openid);
                            $u = M('user')->where(['user_id'=>$user['user_id']])->find();
                            $u['img'] = domain($u['img']);
...
//                   $us['url'] = $url;
    ...
    public function login(){
        ...
        $user = M('User')->where(['phone'=>$phone,'pwd'=>md5($pwd)])->find();
        if ($user){
            if ($user['is_del']==2){
                error('账号被限制,请联系平台!');
            }else{
   ...
   public function register_new(){
       $phone = I('phone');  $yzm = I('yzm');  $pwd = I('pwd');  $log = I('log');  $lag = I('lag');
       (empty($phone) || empty($yzm)) ? error('参数错误!') : true;
       $user = M('User')->where(['phone'=>$phone])->find();
       $user ? error('已注册!') : true;
       $code = M('Mobile_sms')->where(['phone'=>$phone,'state'=>1])->order('intime desc')->limit(1)->find();
