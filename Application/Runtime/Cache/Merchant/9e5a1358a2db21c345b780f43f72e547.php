...
<div class="page-container">
    <div class="text-l">
        <form class="search"  method="get">
            <input type="text" class="input-text" style="width:200px" placeholder="优惠卷名称" value="<?php echo ($_REQUEST["title"]); ?>" name="title">
            <button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜索</button>
            <span style="float:right;padding:0px 10px 10px 0" >
                <a href="<?php echo U('Goods/coupon_create');?>" class=" btn btn-primary radius" >
					<i class="Hui-iconfont">&#xe600;</i>添加优惠卷
				</a>
            </span>
        </form>
    </div>
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <tr class="text-c">
                ...
                <th width="40">ID</th>
                <th width="150">优惠卷名称</th>
                <th width="60">优惠卷金额</th>
                <th width="60">满减</th>
                <th width="60">剩余数量</th>
                <th width="60">发行数量</th>
                <th width="250">有效期</th>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                <td><input name="checkbox" type="checkbox" value="<?php echo ($vo["coupon_id"]); ?>"></td>
                <td><?php echo ($vo["coupon_id"]); ?></td>
                <td><?php echo ($vo["title"]); ?></td>
                <td><u><?php echo ($vo["value"]); ?></u></td>
                <td><?php echo ($vo["limit_value"]); ?></td>
                <td><?php echo ($vo["balance"]); ?></td>
                <td><?php echo ($vo["number"]); ?></td>
                <td><?php echo (date('Y-m-d H:i:s',$vo['start_strtotime'])) ?> | <?php echo (date('Y-m-d H:i:s',$vo['end_strtotime'])) ?></td>
                <td class="td-status">
                    <?php if($vo['status'] == 2): ?><span class="label label-success radius">已发布</span>
                    <?php else: ?>
                    <span class="label label-defaunt radius">已下架</span><?php endif; ?>
                </td>

                <td class="td-manage">
                    <?php if($vo['status'] == 1): ?><a style="text-decoration:none"  onClick="start(this,'<?php echo ($vo["coupon_id"]); ?>')" href="javascript:;" title="发布"><i class="Hui-iconfont">&#xe603;</i></a>
                    <?php else: ?>
                    <a style="text-decoration:none"  onClick="stop(this,'<?php echo ($vo["coupon_id"]); ?>')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a><?php endif; ?>

                    <a style="text-decoration:none" class="ml-5"  href="<?php echo U('goods/coupon_create',['id'=>$vo['coupon_id']]);?>" title="查看详情">
                        <i class="Hui-iconfont">&#xe6df;</i>
                    </a>
                    <a style="text-decoration:none" class="ml-5" onClick="picture_del(this,<?php echo ($vo["coupon_id"]); ?>)" href="javascript:;" title="删除">
                        <i class="Hui-iconfont">&#xe6e2;</i>
                    </a>
                </td>
...
<script type="text/javascript">
    ...
    function datadel(kid){
        ...
        $.post("<?php echo U('goods/coupon_del');?>", {ids:kid}, function(data){
                console.log(data);
    ...
    function stop(obj,id){
        layer.confirm('确认要下架吗？',function(index){
            $.post("<?php echo U('Goods/coupon_state');?>",{id:id},function(data){
                if(data.data.info==1){
    ...
    function picture_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Goods/coupon_del');?>", {ids:id}, function(data){
                if( data.status == 'ok' ){
