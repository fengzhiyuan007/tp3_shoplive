...
<div class="page-container">
	<div class="text-l">
		<form class="search"  method="get">
			...
			<input type="text" class="input-text" style="width:200px" name="order_no" value="<?php echo ($_REQUEST["order_no"]); ?>" id="order_no" placeholder="搜索订单号/下单会员/收件人">
	...
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
				...
				<td class="td-manage">
					<a title="订单详情" class="check_lock" data="<?php echo ($vo["order_no"]); ?>" href="<?php echo U('Horder/order_view',array('id'=>$vo['order_no']));?>"   style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
...
<script type="text/javascript">
    ...
    function datadel(kid){
        ...
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('del_order');?>", {ids:kid}, function(data){
                if( data.status == 'ok' ){
    ...
    function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('del_order');?>", {ids:id}, function(data){
                if( data.status == 'ok' ){
    ...
    function getnums(){
        var num = $("#nus").val();
        window.location.href="/Merchant/Horder/index.html&num="+num;
    }
