...
<div class="page-container">
    <div class="text-l">
        <form class="search"  method="get">
            <input type="text" class="input-text" style="width:200px" placeholder="模板名称" value="<?php echo ($_REQUEST["name"]); ?>" name="name">
            ...
            <span style="float:right;padding:0px 10px 10px 0" >
                <a href="<?php echo U('Goods/postage_create');?>" class=" btn btn-primary radius" >
    ...
    <div class="mt-20">
        <table style="" class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                <td><input type="checkbox" value="<?php echo ($vo["postage_id"]); ?>" name="checkbox"></td>
                <td><?php echo ($vo["postage_id"]); ?></td>
                <td><?php echo ($vo['postage_name']); ?></td>
                <td>
                    <?php if( $vo["postage_way"] == 1): ?>自定义运费<?php else: ?>卖家承担运费<?php endif; ?>
                </td>
                <td>
                    <?php if($vo['postage_type'] == 1): ?>按件
                    <?php elseif($vo['postage_type'] == 2): ?>
                    按重
                    <else>
                    按体积<?php endif; ?>
                <td>
                    <?php if($vo['default'] == 1): ?><u><a href="javascript:;;"><span class="label label-success radius">默认</span></a></u>
                    <?php else: ?>
                    <u><a href="javascript:;;" onClick="default_postage(this,<?php echo ($vo["postage_id"]); ?>)"><span class="label label-defaunt radius">设置默认</span></a></u><?php endif; ?>
                </td>
                <td class="td-manage">
                    <a title="编辑" href="<?php echo U('Goods/postage_create',['id'=>$vo['postage_id']]);?>"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
                    <a title="删除" href="javascript:;" onclick="del(this,'<?php echo ($vo["postage_id"]); ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
                </td>
...
<script type="text/javascript">
    function default_postage(s,v){
        var url = "<?php echo U('Goods/default_postage');?>";
    ...
    function datadel(kid){
        ...
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Goods/del_postage');?>", {ids:kid}, function(data){
                if( data.status == 'ok' ){
    ...
    function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Goods/del_postage');?>", {ids:id}, function(data){
                if( data.status == 'ok' ){
