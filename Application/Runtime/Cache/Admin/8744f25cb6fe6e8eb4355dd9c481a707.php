...
<div class="page-container">
    <div class="text-l">
        <form class="search"  method="get">
            ...
            <input type="text" class="input-text" style="width:250px" placeholder="搜索活动" value="<?php echo ($_GET['username']); ?>" name="username">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        <span class="l">
            ...
            <a href="<?php echo U('Live/add_activity');?>"  class="btn btn-primary radius">
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="text-c">
                    <td><input type="checkbox" value="<?php echo ($l["a_id"]); ?>" name="checkbox"></td>
                    <td><?php echo ($l["a_id"]); ?></td>
                    <td><?php echo ($l["title"]); ?></td>
                    <td><img  src="<?php echo ($l["img"]); ?>" style="width: 50px;height: 50px;border-radius:50%"></td>
                    <td><?php echo ($l["username"]); ?></td>
                    <td><?php echo ($l["tag"]); ?></td>
                    <td><?php echo ($l["count"]); ?></td>
                    <td>
                        <?php switch($l["type"]): case "1": ?>预播<?php break;?>
                            <?php case "2": ?>直播<?php break;?>
                            <?php case "3": ?>录播<?php break;?>
                            <?php case "4": ?>视频<?php break;?>
                            <?php case "5": ?>图片<?php break;?>
                            <?php case "6": ?>文章<?php break;?>
                            <?php default: ?>default<?php endswitch;?>
                    </td>
                    <!-- <td class="state">
                        <?php if($l['state'] == 2): ?><u><a href="javascript:;;" onClick="change_review(this,<?php echo ($l["a_id"]); ?>)"><span class="label label-success radius">已审核</span></a></u>
                        <?php elseif($l['state'] == 1): ?>
                            <u><a href="javascript:;;" onClick="change_review(this,<?php echo ($l["a_id"]); ?>)"><span class="label label-defaunt radius">未审核</span></a></u>
                        <?php else: ?>
                            <u><a href="javascript:;;" ><span class="label label-danger radius">驳回</span></a></u><?php endif; ?>
                    </td> -->
                    <td><?php echo (date("Y-m-d H:i",$l["intime"])); ?></td>
                    <td><?php echo ($l["sort"]); ?></td>
                    <td class="td-manage" style="text-align: center">
                        <?php if($l["state"] != 3): ?><a   onClick="member_stop(this,'<?php echo ($l["a_id"]); ?>')" href="javascript:;" title="驳回"><i class="Hui-iconfont">&#xe6dd;</i></a>
                        <?php else: ?>
                            <a   onClick="member_start(this,'<?php echo ($l["a_id"]); ?>')" href="javascript:;" title="恢复"><i class="Hui-iconfont">&#xe6e1;</i></a><?php endif; ?>
                        <a title="编辑" href="<?php echo U('edit_activity',['id'=>$l['a_id']]);?>"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
                    <?php if($l["url"] != '' AND $l["type"] == 4 ): ?><a title="播放" href="javascript:;" onclick="view_url('<?php echo ($l["url"]); ?>')"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e6;</i></a>
                    <?php elseif($l["url"] != '' AND $l["type"] == 1 ): ?>
                        <a title="播放" href="javascript:;" onclick="view_url('<?php echo ($l["url"]); ?>')"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e6;</i></a><?php endif; ?>    
                        <a title="删除" href="javascript:;" onclick="del(this,'<?php echo ($l["a_id"]); ?>')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
                    </td>
...
<script type="text/javascript">
    /*	$(document).ready(function(){
     ...
     function change_review(obj,id){
        layer.confirm('确认要操作吗？',function(index){
            $.post("<?php echo U('change_activity_review');?>",{id:id},function(data){
                console.log(data);
                if(data.data==1){
    ...
    function datadel(kid){
        ...
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Live/del_activity');?>", {ids:kid}, function(data){
                if( data.status == 'ok' ){
    ...
    function member_stop(obj,id){
        layer.confirm('确认要驳回吗？',function(index){
            $.post("<?php echo U('Live/change_activity_shenhe');?>",{id:id},function(data){
                console.log(data);
                if(data.info == 3){
                    $(obj).parent().parent().find(".td-manage").prepend('<a style="text-decoration:none"  onClick="member_start(this,'+id+')" href="javascript:;" title="恢复"><i class="Hui-iconfont">&#xe6e1;</i></a>');
                    $(obj).parent().siblings('.state').html('<u><a href="javascript:;;" ><span class="label label-danger radius">驳回</span></a></u>');
                    $(obj).remove();
                    layer.msg('已驳回成功!',{icon: 5,time:1000});
                }
            },'json')
        });
    }
    ...
    function member_start(obj,id){
        layer.confirm('确认要恢复吗？',function(index){
            $.post("<?php echo U('Live/change_activity_shenhe');?>",{id:id},function(data){
                if(data.info != 3){
                    $(obj).parent().parent().find(".td-manage").prepend('<a style="text-decoration:none" class="ml-5" onClick="member_stop(this,'+id+')" href="javascript:;" title="驳回"><i class="Hui-iconfont">&#xe6dd;</i></a>');
                    $(obj).parent().siblings('.state').html('<u><a href="javascript:;;" onClick="change_review(this,'+id+')"><span class="label label-defaunt radius">未审核</span></a></u>');
                    $(obj).remove();
                    layer.msg('已恢复成功!',{icon: 6,time:1000});
    ...
    function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Live/del_activity');?>", {ids:id}, function(data){
                if( data.status == 'ok' ){
