...
function getnums(){
    ...
    window.location.href="/Admin/Caiwu/income_withdraw?nums="+num+"&start="+start+"&end="+end+"&username="+username+"&status="+status;
}
function sendname() {
    var num = $("#nus").val();
    var status = $("#status").val();
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <!--<td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["user_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>-->
    ...
    <td align="center" class="onerow"><?php if( $l["type"] == 1 ): ?><button onclick="tx(this,'<?php echo ($l["withdraw_income_id"]); ?>')" type="button" class="btn btn-success radius">确认结算</button><?php else: ?><button type="button" class="btn btn-primary radius">已结算</button>
            </a><?php endif; ?></td>
...
function tx(obj,id){
    layer.confirm('确认完成吗？',function(index){
        $.post("<?php echo U('tx_income');?>", {ids:id}, function(data){
            if( data.status == 'ok' ){
