<?php if (!defined('THINK_PATH')) exit(); $uid = I('goods_uuid'); $uid?$text=$re['goods_name']:$text='添加商品'; ?>
...
<div class="page-content">
	...
	<div class="col-xs-12" style="display: block;margin-bottom: 15px;">
		...
		<div class="all tag1">
			...
			<div class="row cl hide">
				...
				<select name="brand_id" id="brand"  class="select">
					...
					<?php if(is_array($brand)): $i = 0; $__LIST__ = $brand;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["brand_id"]); ?>" <?php if($re['brand_id'] == $vo['brand_id']): ?>selected<?php endif; ?>><$vo['brand_name']></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
				</span>
				</div>
			</div>
			...
			<select name="seed_class" id="seed_class" class="select">
				...
				<?php if(!empty($re['seed_class'])): if(is_array($seed_class)): foreach($seed_class as $key=>$v): ?><option value="<?php echo ($v["class_id"]); ?>" <?php if( $re['seed_class'] == $v['class_id']): ?>selected<?php endif; ?>><?php echo ($v["class_name"]); ?></option><?php endforeach; endif; endif; ?>
			</select>
			</span> </div>
			</div>
			<!--<div class="row cl">-->
			...
			<!--<?php if(!empty($re['second_category'])): ?>-->
			<!--<?php if(is_array($second_category)): foreach($second_category as $key=>$v): ?>-->
			<!--<option value="<?php echo ($v["id"]); ?>" <?php if( $re['second_category'] == $v['id']): ?>selected<?php else: endif; ?>><?php echo ($v["category"]); ?></option>-->
			...
			<!--<div class="formControls col-xs-8 col-sm-9">-->
			<!--<input type="text" name="sale_number" id="sale_number" placeholder="购买人数" value="" class="input-text" style="width:90%">-->
			<!--元</div>-->
			<!--</div>-->
			<div class="row cl">
		...
		<div class="all Height0 tag2">
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">图片：(750*750)</label>
				<div class="formControls col-xs-8 col-sm-9">
			...
			<div class="row cl" style="margin-bottom: 50px;">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<div class="droparea spot" id="image1" style="background-image: url('<?php echo ($re['imgs'][0]); ?>');background-size: 220px 160px;" >
						...
						<input type="hidden" name="imgss" id="image_A" value="<?php echo ($re['imgs'][0]); ?>" />
						<div class="instructions" style="top: 135px;color: red;">建议尺寸750*750,图片小于2M</div>
					</div>
					...
					<div class="droparea spot" id="image3" style="background-image: url('<?php echo ($re['imgs'][2]); ?>');background-size: 220px 160px;" >
						...
						<div id="uparea3"></div>
						<input type="hidden" name="imgs[]" id="image_3" value="<?php echo ($re['imgs'][2]); ?>" />						<div class="instructions" style="top: 135px;color: red;">建议尺寸750*750,图片小于2M</div>
						<div class="instructions" style="top: 135px;color: red;">建议尺寸750*750,图片小于2M</div>
					</div>
		...
		<div style="" class="all Height0 tag3">
			...
			<!--<div class="formControls col-xs-8 col-sm-9">-->
			<!--<textarea name="param"  id="content2" style='width:100%;height:150px;'  placeholder="说点什么...最少输入10个字符" ></textarea>-->
			<!--</div>-->
			<!--</div>-->
		</div>
		...
		<div class="tabBar cl">
			...
			<span class="select-box radius">
				...
				<!--<a onClick="addGoodsTag()" href="javascript:;;" title="添加自定义属性" class="btn btn-default radius" type="button">-->
					<!--<i class="Hui-iconfont">&#xe600;</i>自定义-->
				<!--</a>-->
			</div>
		</div>
			...
			<div class="cl pd-30 bg-1 bk-gray mt-20">
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<?php if(is_array($goods_specification_relation)): $i = 0; $__LIST__ = $goods_specification_relation;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c tr_specification">
						...
						<!--<td><input type="text" name="specification_sale_ratio[]"  onblur="checkRatio($(this))"  placeholder="销售比例" value="<?php echo ($vo["sale_ratio"]); ?>" class="input-text tc" style="width:90%"></td>-->
						...
						<td>
							...
							<!--<input name="video" value="" id="video" type="file" accept="image/*">点击这里上传图片-->
							<!--&lt;!&ndash;<?php echo ($_GET['id']?'点击这里更换视频':'点击这里上传视频'); ?>&ndash;&gt;-->
							<!--</div>-->
...
//                '<td><input type="text" name="specification_sale_ratio[]"  placeholder="销售比例" value='+str1.sale_ratio+' class="input-text tc" style="width:90%"></td>' +
    ...
    function del_stock(id,v){
        var url = "<?php echo U('del_goods_stock');?>";
        if(!confirm('确定要删除该记录？'))
            return false;
    ...
    function add_stock(id,v){
        var url = "<?php echo U('edit_goods_stock');?>";
        if(id != null || id != undefined || id != ' ' || id != '0') {
            $.ajax({
                type: "GET",
                url: url,
