...
<script type="text/javascript">
 $(function($) {
	$("#file_upload2").uploadify({
	 		...
	 		'folder'         : '/Public/admin/Uploads/music',
	 		'queueID'        : 'fileQueue2',
	 		'sizeLimit'      :	10 * 1000 * 1024,
			'buttonImg'      : '/Public/admin/images/llsc.jpg',
			'width'          :  85,
			'height'          :  28,
	 		'fileExt'        : '*.mp3;*.MP3;*.wav;', //允许文件上传类型,和fileDesc一起使用.
	 		'fileDesc'       : '*.mp3;*.MP3;*.wav;',  //选择文件对话框中的提示文本.
	 		'auto'           : true,
	 		'multi'          : false,	
...
<!-----------------------------------------内容开始--------------------------------------------------->
<div class="infoBox">
<form name="form" action="<?php echo U('doadd_music');?>"  method="post" onsubmit="return checkss();">
<table width="90%" border="0" cellpadding="0" cellspacing="0" id="basic">
               <input type="hidden" value="<?php echo ($u["music_id"]); ?>" name="id">
                <tr>
                  <td  width="2%"  style="text-align:right;color:#2d52a5"><div style="width:55px;">音乐:</div></td>
		  		  <td colspan="2">
		  		  <div class="upimg-box"><div id="fileQueue2"></div>
                  <input id="file_upload2" type="file" />
                  &nbsp;&nbsp;<input name="url" type="text" id="fileDoc3" style="width: 70%" value="<?php echo ($u["path"]); ?>" />&nbsp;<span>上传mp3</span>
                 </div>
		  		  </td>
                </tr>
