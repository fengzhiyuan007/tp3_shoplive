...
function getnums(){
    ...
    var end = $("#end").val();
    var name = $("#name").val();
    window.location.href="/Admin/Business/apply_list?nums="+num+"&start="+start+"&end="+end+"&name="+name;
}
function sendname() {
    var num = $("#nus").val();
    var start = $("#start").val();
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["merchants_id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["merchants_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>
    ...
    <td align="center" class="onerow"><img src="<?php echo ($l["img"]); ?>" style="width:50px; height:50px; border-radius:25px;"></td>
    ...
    <td align="center" class="onerow"><?php echo ($l["update_time"]); ?></td>


  <td align="center">
                  <a href="<?php echo U('edit_merchants',array('mid'=>$l['member_id']));?>">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
...
function del(kid){
    ...
    if(!confirm('确定删除？'))
        return false;
    $.post("<?php echo U('del_merchants');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('apply_list');?>");
        }else{
            alert('删除失败！');
        }
    });
