...
function getnums(){
    var num = $("#nus").val();
    window.location.href="/Admin/Base/lebel?nums="+num;
}
...
<div class="tools"> 
<div class="add"><span><a href="/Admin/Base/toadd_lebel">添加</a></span></div>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["lebel_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>
    <td align="center" class="onerow"><?php if( $l["type"] == 1 ): ?>直播<?php else: ?>资讯<?php endif; ?></td>
    <td align="center" class="onerow"><?php echo ($l["name"]); ?></td>
    <td align="center" class="onerow"><?php echo (date("Y-m-d H:i",$l["intime"])); ?></td>

  <td align="center">
                  <a href="<?php echo U('toadd_lebel',array('id'=>$l['lebel_id']));?>">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                  <a href="javascript:;"  onclick="del(<?php echo ($l["lebel_id"]); ?>);">删除</a>
...
function del(kid){
    ...
    if(!confirm('确定删除？'))
        return false;
    $.post("<?php echo U('del_lebel');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('lebel');?>");
        }else{
            alert('删除失败！');
        }
    });
