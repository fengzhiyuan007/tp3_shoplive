...
function getnums(){
    ...
    window.location.href="/Admin/Caiwu/shop_withdraw?nums="+num+"&start="+start+"&end="+end+"&username="+username+"&status="+status;
}
function sendname() {
    var num = $("#nus").val();
    var status = $("#status").val();
...
<!--<input name="dele" type="submit" value="删除" onclick="del();" class="wr"   style="border:none; background-color:#F2F7FD; color:#2D52A5;margin-top:3px;" /></a></span></div>-->
    ...
    <span style="float:right;padding-right:50px;padding-top:5px;">
        状态: <select name="status" id="status" onchange="sendname()">
            ...
            <option value="3" <?php if( $status == 3 ): ?>selected<?php endif; ?>>已结算</option>
...
<tr class="tabTitleMain">
    ...
    <td width="10%" align="center">银行卡号</td>
    <td width="10%" align="center">银行名称</td>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <!--<td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["user_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>-->
    ...
    <td align="center" class="onerow"><?php echo ($l["contact_mobile"]); ?></td>
    ...
    <td align="center" class="onerow"><?php if( $l["status"] == 1 ): ?>待结算<?php elseif( $l["status"] == 3 ): ?>已结算<?php else: ?>失败<?php endif; ?></td>
    <td align="center" class="onerow"><?php echo (date("Y-m-d H:i",$l["intime"])); ?></td>

    <td align="center" class="onerow"><?php if( $l["status"] == 1 ): ?><button onclick="tx_shop(this,'<?php echo ($l["withdraw_id"]); ?>')" type="button" class="btn btn-success radius">确认结算</button><?php else: ?><button type="button" class="btn btn-primary radius">已结算</button>
            </a><?php endif; ?></td>
...
<script type="text/javascript">
function tx_shop(obj,id){
    layer.confirm('确认完成吗？',function(index){
        $.post("<?php echo U('tx_shop');?>", {ids:id}, function(data){
            if( data.status == 'ok' ){
