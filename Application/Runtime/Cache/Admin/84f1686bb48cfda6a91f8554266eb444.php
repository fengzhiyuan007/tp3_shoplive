...
function getnums(){
    ...
    var end = $("#end").val();
    var name = $("#name").val();
    window.location.href="/Admin/Goods/goods_list?nums="+num+"&start="+start+"&end="+end+"&name="+name;
}
function sendname() {
    var num = $("#nus").val();
    var start = $("#start").val();
	...
	<div class="mt-20">
		<table style="" class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
					...
					<!-- <td><img src="<?php echo ($vo["goods_qrcode"]); ?>" onclick="view_play_img('<?php echo U('goods_qrcode',['goods_id'=>$vo.goods_id]);?>')" style="width:50px; height:50px; border-radius:25px;"></td> -->
					<td><a href="<?php echo U('edit_goods',['goods_uuid'=>$vo['goods_uuid']]);?>"><u style="cursor:pointer" class="text-primary" ><?php echo ($vo["goods_name"]); ?></u></a></td>
					...
					<td>
						<?php if($vo['is_review'] == 1): ?><u><a href="javascript:;;" onClick="change_review(this,<?php echo ($vo["goods_id"]); ?>)"><span class="label label-success radius">已审核</span></a></u>
						<?php else: ?>
						<u><a href="javascript:;;" onClick="change_review(this,<?php echo ($vo["goods_id"]); ?>)"><span class="label label-defaunt radius">未审核</span></a></u><?php endif; ?>
					</td>
					<td class="">
							<a href="javascript:void(0)" onclick="change_tuijian(<?php echo ($vo['goods_id']); ?>,this)">
					...
					<td class="td-manage">
						<?php if($vo['goods_state'] == 1): ?><a style="text-decoration:none"  onClick="member_stop(this,'<?php echo ($vo["goods_id"]); ?>')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a>
							<?php else: ?>
						...
						<a title="编辑" href="<?php echo U('edit_goods',['goods_uuid'=>$vo['goods_uuid']]);?>"  class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
...
/*	$(document).ready(function(){
	...
	function datadel(kid){
		...
		layer.confirm('确认要删除吗？',function(index){
			$.post("<?php echo U('del_goods');?>", {ids:kid}, function(data){
				if( data.status == 'ok' ){
