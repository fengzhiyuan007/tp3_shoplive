...
function getnums(){
    ...
    window.location.href="/Admin/Business/warehouse?nums="+num+"&start="+start+"&end="+end+"&name="+name+"&is_all="+is_all;
}
function sendname() {
    var num = $("#nus").val();
    var start = $("#start").val();
    var end = $("#end").val();
    var name = $("#name").val();
    window.location.href="/Admin/Business/warehouse?nums="+num+"&start="+start+"&end="+end+"&name="+name;
}
</script>

<form action="javascript:;" method="post">
<div class="tools"> 
<div class="add"><span><a href="/Admin/Business/toadd_warehouse">添加总仓</a></span></div>
...
function del(kid){
    ...
    if(!confirm('确定删除？'))
        return false;
    $.post("<?php echo U('del_warehouse');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('warehouse');?>");
        }else{
            alert('删除失败！');
        }
    });
