...
function getnums(){
    ...
    var end = $("#end").val();
    var name = $("#name").val();
    window.location.href="/Admin/Goods/specifications?nums="+num+"&start="+start+"&end="+end+"&name="+name;
}
function sendname() {
    var num = $("#nus").val();
    var start = $("#start").val();
...
<input name="dele" type="submit" value="删除" onclick="datadel()" class="wr"   style="border:none; background-color:#F2F7FD; color:#2D52A5;margin-top:3px;" /></a></span></div>
    ...
    <span style="float:right;padding-top:4px;" >
        <a href="javascript:;;" class="btn btn-primary radius" onclick="category_edit('添加规格（First）','<?php echo U('edit_specifications');?>','4','','510')" >
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["specification_id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  ...
  <td align="center" class="td-manage">
        ...
        <a style="text-decoration:none" class="ml-5"  href="javascript:;;" onclick="category_edit('编辑规格（First）','<?php echo U('edit_specifications',['id'=>$l['specification_id']]);?>','4','','510')" title="编辑">
            <i class="Hui-iconfont">&#xe6df;</i>
        </a>
