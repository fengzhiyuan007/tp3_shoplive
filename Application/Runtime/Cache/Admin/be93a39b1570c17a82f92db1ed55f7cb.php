    ...
    <div class="cl pd-5 bg-1 bk-gray">
        <span class="l">
            <a href="javascript:;" onclick="datadel()" class="btn btn-danger radius">
                ...
                批量删除</a>
            <a class="btn btn-primary radius"  href="<?php echo U('Home/add_home_class');?>">
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                ...
                <td><a href="<?php echo U('Home/edit_home_class',['id'=>$vo['id']]);?>"><u><?php echo ($vo["title"]); ?></u></a></td>
                <td><a href="javascript:;"><img width="100" class="picture-thumb" src="<?php echo ($vo["img"]); ?>"></a></td>
                <td>
                    <a href="javascript:void(0)" onclick="plus(<?php echo ($vo['id']); ?>)" title="上移">
                        <i class="Hui-iconfont">&#xe679;</i>
                    </a>
                    <a href="javascript:void(0)" onclick="minus(<?php echo ($vo['id']); ?>)" title="下移">
                ...
                <td class="td-manage">
                    ...
                    <a style="text-decoration:none" class="ml-5"  href="<?php echo U('Home/edit_home_class',['id'=>$vo['id']]);?>" title="编辑">
                        <i class="Hui-iconfont">&#xe6df;</i>
                    </a>
                    <a style="text-decoration:none" class="ml-5" onClick="picture_del(this,<?php echo ($vo["id"]); ?>)" href="javascript:;" title="删除">
                        <i class="Hui-iconfont">&#xe6e2;</i>
                    </a>
                </td>
...
<script type="text/javascript">
    ...
    function datadel(kid){
        ...
        $.post("<?php echo U('Home/del_home_class');?>", {ids:kid}, function(data){
            if( data.status == 'ok' ){
                console.log(data);
    ...
    function picture_stop(obj,id){
        layer.confirm('确认要下架吗？',function(index){
            $.post("<?php echo U('Home/change_class_status');?>",{id:id},function(data){
                if(data.data.info==1){
    ...
    function plus(v){
        $.post("<?php echo U('Home/plus_class_sort');?>",{id:v},function(data){
            if(data['status'] == 'ok'){
    ...
    function minus(v){
        $.post("<?php echo U('Home/minus_class_sort');?>",{id:v},function(data){
            if(data['status'] == 'ok'){
                layer.msg(data.data,{icon:1,time:2000},function(){
                    window.location.href = window.location.href;
    ...
    function picture_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('Home/del_home_class');?>", {ids:id}, function(data){
                if( data.status == 'ok' ){
