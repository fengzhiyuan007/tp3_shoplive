    ...
    <div class="text-l">
        <form class="search"  method="get">
            ...
            <select name="refund_state"  class="select select-box inlin" id="refund_state" style="width:120px">
                ...
                <option value="wait_review" <?php if($_REQUEST["refund_state"] == 'wait_review'): ?>selected<?php endif; ?>>待支付</option>
                <option value="accept" <?php if( $_REQUEST["refund_state"] == 'accept'): ?>selected<?php endif; ?>>接受</option>
                <option value="refuse" <?php if($_REQUEST["refund_state"] == 'refuse'): ?>selected<?php endif; ?>>拒绝</option>
                <option value="end" <?php if($_REQUEST["refund_state"] == 'end'): ?>selected<?php endif; ?>>结束</option>
            </select>
            </span>
            <span class="select-box" style="width:150px">
            <select name="refund_type"  class="select select-box inlin" id="refund_type" style="width:120px">
                <option value="">请选择类型</option>
                <option value="1" <?php if($_REQUEST["refund_type"] == '1'): ?>selected<?php endif; ?>>换货</option>
                <option value="2" <?php if( $_REQUEST["refund_type"] == '2'): ?>selected<?php endif; ?>>退货</option>
            </select>
            </span>
            <input type="text" class="input-text" style="width:200px" name="refund_no" value="<?php echo ($_REQUEST["refund_no"]); ?>" id="refund_no" placeholder="搜索售后单号/订单号/申请人">
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-c">
                    <td><input type="checkbox" value="<?php echo ($vo["refund_id"]); ?>" name="checkbox"></td>
                    <td><?php echo ($vo["refund_no"]); ?></td>
                    <td><a href="<?php echo U('order_view',['id'=>$vo['order_no']]);?>"><u style="cursor:pointer" class="text-primary" ><?php echo ($vo["order_no"]); ?></u></a></td>
                    <td><?php echo ($vo["username"]); ?></td>
                    <td><?php echo ($vo["phone"]); ?></td>
                    <!--<td><?php echo ($vo['cost']); ?></td>-->
                    <td>
                        <?php switch($vo['refund_type']): case "1": ?>退款<?php break;?>
                            <?php case "2": ?>退货<?php break; endswitch;?>
                    </td>
                    <td><?php echo ($vo['merchants_name']); ?></td>
                    <td>
                        <?php switch($vo['refund_state']): case "wait_review": ?>待审核<?php break;?>
                            <?php case "accept": ?>接受审核<?php break;?>
                            <?php case "refuse": ?>拒绝<?php break;?>
                            <?php case "end": ?>结束<?php break; endswitch;?>
                    </td>
                    <td><?php echo ($vo["create_time"]); ?></td>
                    <td class="td-manage">
                        <a title="订单详情" class="check_lock" data="<?php echo ($vo["refund_no"]); ?>" href="<?php echo U('refund_view',array('id'=>$vo['refund_no']));?>"   style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
                        <a title="订单删除" href="javascript:;" onclick="del(this,<?php echo ($vo["refund_id"]); ?>);" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
                    </td>
...
<script type="text/javascript">
    ...
    function datadel(kid){
        ...
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('del_refund');?>", {ids:kid}, function(data){
                if( data.status == 'ok' ){
    ...
    function del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.post("<?php echo U('del_refund');?>", {ids:id}, function(data){
                if( data.status == 'ok' ){
    ...
    function getnums(){
        var num = $("#nus").val();
        window.location.href="/Admin/horder/refund/ids/482&num="+num;
    }
