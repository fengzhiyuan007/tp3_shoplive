...
class BaseController extends CommonController {
	...
	function banner(){
		...
		$p = getpage($count,10);
		$ban = M('Banner')->limit($p->firstRow.','.$p->listRows)->order('b_intime desc')->select();
        foreach ($ban as $k=>$v){
	...
	function doadd(){
        ...
        $rules = array(
             array('logos','require','banner图片必须上传！'), 
             array('title','require','标题必须填写！'), 
             array('remark','require','排序必须填写！'), 
...
//            $data['url']      = I('url');
	...
	public function gift_list(){
        ...
        $p = getpage($count,$nus);
        $list =  M('Gift')->limit($p->firstRow.','.$p->listRows)->where($data)->order('intime desc')->select();
    ...
    public function price_list(){
        ...
        $p = getpage($count,$nus);
        $list =  M('Price')->limit($p->firstRow.','.$p->listRows)->where($data)->order('price asc')->select();
        $this->assign('list',$list);
    ...
    public function approve_rule(){
        ...
        $p = getpage($count,$nus);
        $list =  M('Approve_rule')->limit($p->firstRow.','.$p->listRows)->where($data)->order('grade_start asc')->select();
        $this->assign('list',$list);
    ...
    public function level_control(){
        ...
        $p = getpage($count,$nus);
        $list =  M('Level_control')->limit($p->firstRow.','.$p->listRows)->where($data)->order('grade_start asc')->select();
        $this->assign('list',$list);
     ...
     * @商城配置
     */
    public function mall_config(){
        $this->assign ( 'pagetitle', '商城配置' );
        $id = I('id');
        $merchants_per = I('merchants_per');
        $goods_per = I('goods_per');
        if (empty($id)){
            $base = M('system')->field('id,merchants_per,goods_per')->where(['id'=>1])->find();
            $this->assign('base',$base);
            $this->display();
        }else{
            M('system')->where(['id'=>$id])->save(['merchants_per'=>$merchants_per,'goods_per'=>$goods_per]) ? $this->success('成功!',U('mall_config')) : $this->error('失败!',U('mall_config')) ;
        }
    } 
