...
class CommonController extends Controller {
    ...
    public function upload(){
        ...
        if($v[0] == 'image'){
            ...
            // $en = $end[1];
            $key = md5(uniqid(mt_rand(), true)).'.'.$v[1];
            ...
            // $en = $end[1];
            $key = md5(uniqid(mt_rand(), true)).'.'.$vs[1];
            // $key = $_FILES['img']['name'];
            // 初始化 UploadManager 对象并进行文件的上传。
            $uploadMgr = new UploadManager();
...
//          }
     ...
     * 
     */
    public function buildCode($id=0){
        // $id用来查商户信息 = member_id
        $url = C('IMG_PREFIX');
        $img = 'Public/upload/'.md5(uniqid(mt_rand(), true)).'.png';
        Vendor('phpqrcode.phpqrcode');
        \Qrcode::png($url,$img,500,500,true);
        $code = $url.'/'.$img;
        return $code;
