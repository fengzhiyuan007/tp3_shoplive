...
class CaiwuController extends CommonController {
     ...
     * @商户提现记录
     */
    public function shop_withdraw(){
        if( !M()->autoCheckToken($_POST) ) $this->error('禁止站外提交！');
        unset($_POST['__hash__']);
        $data = [];
        if (!empty($_GET['username'])){
            $data['b.contact_mobile|b.merchants_name|b.merchants_id'] = ['like','%'.$_GET['username'].'%'];
            $this->assign('username',$_GET['username']);
        }
        if (!empty($_GET['status'])){
        ...
        $list =  M('Withdraw')
            ->alias('a')
            ->field('a.*,b.merchants_name,b.merchants_id,b.merchants_img,b.contact_mobile')
            ->join('m_shop_merchants b on a.user_id=b.member_id')
            ->limit($p->firstRow.','.$p->listRows)
            ->where($data)
            ->order('a.intime desc')
     ...
     * @用户销售收益提现记录
     */
    public function income_withdraw(){
        if( !M()->autoCheckToken($_POST) ) $this->error('禁止站外提交！');
        unset($_POST['__hash__']);
        $data = [];
        if (!empty($_GET['username'])){
        ...
        $this->assign("nus",$nus);
        $count = M('Withdraw_income')->alias('a')
            ->join('__USER__ b on a.user_id=b.user_id')
            ->where($data)
            ->count();//一共有多少条记录
        $p = getpage($count,$nus);
        $list =  M('Withdraw_income')
            ->alias('a')
            ->field('a.*,b.username,b.ID,b.img,b.phone')
            ->join('__USER__ b on a.user_id=b.user_id')
            ->limit($p->firstRow.','.$p->listRows)
        ...
        $this->assign('list',$list);
        $this->assign("show",$p->show());
        $this->assign ( 'pagetitle', '销售收益提现记录' );
    ...
    public function tx_shop(){
        if(IS_AJAX) {
            $ids = I('ids');
            $data['uptime'] = time();
            $data['status'] = 3;
            $result = M('withdraw')->where(array('withdraw_id'=>$ids))->save($data);
    ...
    public function tx_income(){
        if(IS_AJAX) {
            $ids = I('ids');
            $data['uptime'] = time();
            $data['type'] = 2;
            $result = M('withdraw_income')->where(array('withdraw_income_id'=>$ids))->save($data);
            if($result){
                success(['info'=>'成功']);
            }else{
                error("失败");
    ...
    public function goods_settlement(){
        ...
        $list = M('shop_per')->alias('a')
            ->field('a.*,b.order_no,c.goods_num,c.goods_name,c.seller')
            ->join('m_shop_order_merchants b ON a.order_merchants_id = b.order_merchants_id')
            ->join('m_shop_order_goods c ON a.order_goods_id = c.order_goods_id')
            ->order("a.time desc")
            ->where($map)
