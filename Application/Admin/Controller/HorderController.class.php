...
class HorderController extends CommonController
{
    ...
    protected function merchant(){
        $merchant = M('shop_merchants')
                    ->where(['is_delete'=>'0','apply_state'=>'2'])
                    ->select();
        return $merchant;
    }
...
//        $map['merchants_id'] = $merchant['member_id'];
    ...
    public function is_del_order(){
        ...
        $map['a.order_type'] = 'goods';
        $merchants_id = I('merchants_id');
        !empty($merchants_id) && $map['merchants_id'] = I('merchants_id');
        $order_no = I('order_no');
...
//                    }
        ...
        if(IS_AJAX){
            ...
            if($check) {
                ...
                if($state == 'end'){
                    ...
                    $res = $this->return_order($order_merchants['order_merchants_id']);
    ...
    //支付宝退款接口
    public function return_ali($out_trade_no,$trade_no){
        require_once "./ThinkPHP/Library/Vendor/alipay/AopSdk.php";
        require_once "./ThinkPHP/Library/Vendor/alipaywap/config.php";
        require_once "./ThinkPHP/Library/Vendor/alipaywap/pagepay/service/AlipayTradeService.php";
        require_once "./ThinkPHP/Library/Vendor/alipaywap/pagepay/buildermodel/AlipayTradeRefundContentBuilder.php";
        ...
        $trade_no = $trade_no;
        //请二选一设置
        $pay_charge = M('shop_order_merchants')->where(['pay_no'=>$out_trade_no])->getField('pay_charge');
        $res = json_decode($pay_charge, true);

        //需要退款的金额，该金额不能大于订单金额，必填
        $refund_amount = $res['receipt_amount'];
    ...
    //订单退款
    public function return_order($order_merchants_id){
            ...
            if ($order_merchants['order_state'] != 'wait_send') {
                $this->error("退款状态不符合，请选择售后服务");
            }
            if (time() - strtotime($order_merchants['pay_time']) > 7*24* 3600) {
                $this->error("可退款时间已过，请选择售后服务");
            }

            $pay_charge = json_decode($order_merchants['pay_charge'], true);
            //微信退款 支付宝退款
            switch ($order_merchants['pay_way']) {
                case '微信APP支付':
                    require_once "./ThinkPHP/Library/Vendor/wx/lib/WxPay.Api.php";
                    if(isset($order_merchants["ping_no"]) && $order_merchants["ping_no"] != ""){
                        $transaction_id = $order_merchants["ping_no"];
                        $total_fee = $pay_charge['total_fee'] * 100;
                        $refund_fee = $pay_charge['cash_fee'] * 100;
                    ...
                    break;
                case '微信扫码':
                    require_once "./ThinkPHP/Library/Vendor/wxpub/lib/WxPay.Api.php";
                    if(isset($order_merchants["ping_no"]) && $order_merchants["ping_no"] != ""){
                        $transaction_id = $order_merchants["ping_no"];
                        $total_fee = $pay_charge['total_fee'] * 100;
                        $refund_fee = $pay_charge['cash_fee'] * 100;
