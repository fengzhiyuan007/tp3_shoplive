...
class CliController extends Controller{
    /**
     * 发货之后，7天，客户没确认收货，订单自动改为已确认收货
     */
    public function confirm(){
        $list = M('shop_order_merchants')
              ->where(['order_state'=>'wait_receive','is_delete'=>0,'settlement_state'=>0])
              ->field('order_merchants_id,merchants_id,member_id,order_no,send_time')
              ->select();
        ...
        if($list){
            foreach ($list as $k => &$v) {
                //发货时间一周后
                $time = strtotime("+1 week,".$v['send_time']."");
                if($now > $time){
                    //已经过七天
                    $result = M('shop_order_merchants')->where(['order_merchants_id'=>$v['order_merchants_id']])->save(['order_state'=>'wait_assessment','receive_time'=>date("Y-m-d H:i:s",time())]);

                    if($result){
                        set_message($v['member_id'],'订单发货已过七天，自动签收','2',$v['order_merchants_id']);
                    }
    ...
    /**
     * 确认收货，7天后，结算订单，结算订单不允许退货、退款
     */
    public function settlement(){
        $list = M('shop_order_merchants')
              ->where(['order_state'=>'wait_assessment','is_delete'=>0,'settlement_state'=>0])
              ->field('order_merchants_id,merchants_id,member_id,order_no,receive_time')
              ->select();

        if($list){
            foreach ($list as $k => &$v) {
                //发货时间一周后
                $time = strtotime("+1 week,".$v['receive_time']."");
    ...
    public function send_pop(){
        ...
        for ($i=1;$i<1000;$i++){  
            ...
            if (!empty($data)) {  
                ...
                M('Message')->add(['type'=>1,'user_id2'=>$res['user_id'],'content'=>$no['content'],'title'=>$no['title'],'intime'=>time(),'date'=>date('Y-m-d',time())]);
        ...
        for ($i=1;$i<50;$i++){
            ...
            foreach ($live as $k=>$v){
                if (time() - $v['start_time'] > 10) {
                    ...
                    /**更改活动数据**/
                    M('activity')->where(array('type'=>2,'a_id'=>$v['a_id']))->setField(array('type'=>3,'x_id'=>0,'store_status'=>2));
                    /**更改活动数据**/
                    ...
                    /**更改活动数据**/
                    M('activity')->where(array('type'=>3,'a_id'=>$v['a_id']))->setField(array('x_id'=>$res,'store_status'=>1));
                    /**更改活动数据**/
    ...
    public function push_user(){
        for ($i = 0; $i < 500; $i++) {
            ...
            foreach ($live as $s=>$n){
                ...
                $live[$s]['play_img'] = domain($n['play_img']);
